...
class AppManager extends Component {
  ...
  listApps() {
    ...
    AppService.listApps(session.getAccessToken().getJwtToken())
      .then(response => {
        this.setState({
          apps: response
        });
  ...
  handleOnDeleteApp(appId) {
    ...
    if (confirmation) {
      ...
      AppService.removeApp(appId, token)
        .then(() => {
          AppService.listApps(token)
            .then(apps => {
              this.setState({
                apps
              });
  ...
  handleOnAddApp(app) {
    ...
    AppService.addApp(app, token)
      .then(newApp => {
        AppService.listApps(token)
          .then(apps => {
            console.log("newApp.appName: " + newApp.appName);
            apps.forEach(n =>
              n.appName === newApp.appName
                ? (n.isNew = "true")
                : (n.isNew = undefined)
            );
            this.setState({
  ...
  render() {
    return (
      <div>
        <Modal
          isOpen={this.state.isAddAppModalOpen}
          onRequestClose={this.handleOnCloseAddAppModal}
          style={customStyles}
        >
          <AddForm
            onSaveApp={this.handleOnAddApp}
            onCloseModal={this.handleOnCloseAddAppModal}
          />{" "}
        </Modal>{" "}
        <Modal
          isOpen={this.state.isEditAppModalOpen}
          onRequestClose={this.handleOnCloseEditAppModal}
        >
          <EditForm
            onSaveApp={this.handleOnEditApp}
            onCloseModal={this.handleOnCloseEditAppModal}
            app={this.state.selectedApp}
          />{" "}
        </Modal>{" "}
        <div className="mb-3">
          <ControlPanel
            openAddAppModal={this.handleOpenAddAppModal}
            onFindApps={this.handleOnFindApps}
          />{" "}
        </div>{" "}
        <AppTable
          apps={this.state.apps}
          onDeleteApp={this.handleOnDeleteApp}
          onOpenEditAppModal={this.handleOpenEditAppModal}
        />{" "}
      </div>
    );
  }
